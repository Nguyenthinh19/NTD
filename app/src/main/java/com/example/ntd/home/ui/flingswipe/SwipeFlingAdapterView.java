package com.example.ntd.home.ui.flingswipe;
...
public class SwipeFlingAdapterView extends BaseFlingAdapterView {
    ...
    protected int MAX_VISIBLE = 3;
    protected int MIN_ADAPTER_STACK = 6;
    protected float ROTATION_DEGREES = 15.f;

    protected Adapter mAdapter;
    protected int LAST_OBJECT_IN_STACK = 0;
    protected onFlingListener mFlingListener;
    protected AdapterDataSetObserver mDataSetObserver;
    protected boolean mInLayout = false;
    protected View mActiveCard = null;
    protected OnItemClickListener mOnItemClickListener;
    protected FlingCardListener flingCardListener;
    ...
    */
    ...
    protected void setTopView() {
        if(getChildCount()>0){
            ...
            View contentView = mActiveCard;
            if(contentView != null){
                CardView cardView = (CardView) contentView.findViewById(R.id.cardView_home_job);
                cardView.setCardElevation(convertDpToPixel(6));
                contentView.setScaleX(1);
                contentView.setScaleY(1);
//                up = mActiveCard.getWidth() * 0.05f;
                contentView.setY(contentView.getY() - convertDpToPixel(36));
            }

            contentView = getChildAt(LAST_OBJECT_IN_STACK - 1);
            if(contentView != null){
                CardView cardView = (CardView) contentView.findViewById(R.id.cardView_home_job);
                cardView.setCardElevation(convertDpToPixel(4));
                contentView.setScaleX(0.95f);
                contentView.setScaleY(0.95f);
                contentView.setY(contentView.getY() - convertDpToPixel(18));
                contentView.setX(contentView.getX() + convertDpToPixel(16));
            }

            contentView = getChildAt(LAST_OBJECT_IN_STACK - 2);
            if(contentView != null){
                CardView cardView = (CardView) contentView.findViewById(R.id.cardView_home_job);
                cardView.setCardElevation(convertDpToPixel(2));
                contentView.setScaleX(0.95f);
                contentView.setScaleY(0.95f);
                contentView.setY(contentView.getY() - convertDpToPixel(9));
                contentView.setX(contentView.getX() + convertDpToPixel(6));
            }
